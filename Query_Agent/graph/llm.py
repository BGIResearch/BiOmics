...
def get_llm_by_type(llm_type: LLMType, config_source: str="env") -> ChatOpenAI:
    ...
    if config_source == "env":
        # 获取项目根目录下的配置文件
        project_root = os.getenv('PROJECT_ROOT', '/home/liyuntian/Biomics_agent')
        config_path = os.path.join(project_root, 'graph', 'brick_test_config.env')
        conf = load_env_config(config_path)
    else:
