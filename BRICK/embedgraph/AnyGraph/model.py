...
class Feat_Projector(nn.Module):
    ...
    def svd_proj(self, feats):
        if args.latdim > feats.shape[0] or args.latdim > feats.shape[1]:
            ...
            decom_feats = t.concat([decom_feats, t.zeros([decom_feats.shape[0], args.latdim-dim]).to(args.device)], dim=1)
            s = t.concat([s, t.zeros(args.latdim - dim).to(args.device)])
        else:
...
class Adj_Projector(nn.Module):
    ...
    def svd_proj(self, adj):
        ...
        if args.latdim > adj.shape[0] or args.latdim > adj.shape[1]:
            ...
            svd_u = t.concat([svd_u, t.zeros([svd_u.shape[0], args.latdim-dim]).to(args.device)], dim=1)
            svd_v = t.concat([svd_v, t.zeros([svd_v.shape[0], args.latdim-dim]).to(args.device)], dim=1)
            s = t.concat([s, t.zeros(args.latdim-dim).to(args.device)])
        else:
...
class Expert(nn.Module):
    def __init__(self):
        ...
        self.topo_encoder = TopoEncoder().to(args.device)
        if args.nn == 'mlp':
            self.trainable_nn = MLP().to(args.device)
        else:
            self.trainable_nn = GraphTransformer().to(args.device)
        self.trn_count = 1
    
    def forward(self, projectors, pck_nodes=None):
        embeds = projectors.to(args.device)
    ...
    def cal_loss(self, batch_data, projectors):
        ancs, poss, negs = list(map(lambda x: x.to(args.device), batch_data))
    ...
    def pred_for_test(self, batch_data, cand_size, projectors, rerun_embed=True):
        ancs, trn_mask = list(map(lambda x: x.to(args.device), batch_data))
        if rerun_embed:
            try:
        ...
        mask_mat = t.sparse.FloatTensor(trn_mask, t.ones(trn_mask.shape[1]).to(args.device), t.Size([ancs.shape[0], cand_size]))
    ...
    def attempt(self, topo_embeds, dataset):
        ...
        rows, cols, negs = list(map(lambda x: t.from_numpy(x).long().to(args.device), [dataset.ancs, dataset.poss, dataset.negs]))
        if rows.shape[0] > args.attempt_cache:
            random_perm = t.randperm(rows.shape[0], device=args.device)
    ...
    def assign_experts(self, handlers, reca=True, log_assignment=False):
        ...
        with t.no_grad():
            ...
            for dataset_id, handler in enumerate(handlers):
                topo_embeds = handler.projectors.to(args.device)
                for expert_id, expert in enumerate(self.experts):
                    expert = expert.to(args.device)
                    score = expert.attempt(topo_embeds, handler.trn_dataset)
                    if reca:
